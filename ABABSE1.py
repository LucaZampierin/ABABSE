...
def ababse1(input, sen_len, target, sen_len_tr, aspects, n_aspects, drop_rate1, drop_rate2, sub_vocab, l2, _id='all'):
    ...
    # Attention layer.
    att = bilinear_attention_layer(input, pool_t, sen_len, FLAGS.n_hidden, l2, FLAGS.random_base, 'tl')

    # Sentence representation
    z_s = tf.matmul(att, input)
    z_s = tf.squeeze(z_s, axis=1)

    # Autoencoder-like dimensionality reduction. Sentiment classification.
...
def main(train_path, test_path, test_size, sub_vocab, learning_rate=FLAGS.learning_rate, drop_rate=FLAGS.drop_rate1,
    ...
    with tf.device('/gpu:1'):
        ...
        optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate, beta1=b1, beta2=b2, epsilon=1e-08).minimize(
    ...
    with tf.Session(config=config) as sess:
        ...
        te_x, te_sen_len, te_target_word, te_tar_len, te_y, te_aspect, all_y, te_x_neg, tex_neg_sen_len = load_inputs_twitter(
            test_path,
            word_id_mapping,
            FLAGS.max_sentence_len,
            'IAN',
        ...
        for i in range(FLAGS.n_iter):
            ...
            p = []
            # Test model
            for test, num in get_batch_data(te_x, te_sen_len, te_y, te_target_word, te_tar_len, te_aspect, te_x_neg, tex_neg_sen_len,
                                            1000, 0, 0, 0, False):
